...
func Test_BinaryData_Save(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:     "new binary data",
			id:       0,
			data:     "some binary",
			dataName: "name",
			srcData:  []byte("some binary"),
			owner:    &User{login: "test", password: "password"},
			encryptedData: &EncryptedData{
				Data: []byte("some binary"),
				Key:  []byte("key"),
			},
			successPD:      true,
			successEncrypt: true,
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			entity := newBinaryData(tt.owner, []byte{}, "")
			entity.dataRepository = r
			entity.id = tt.id

			entity.SetData([]byte(tt.data))
...
func Test_findBinaryDataByID(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			actual, err := findBinaryDataByID(ctx, tt.input, tt.user, r)
			...
			assert.Equal(t, TypeBinaryData, actual.Type())
